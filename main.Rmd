...
plotData.lag <- as.data.frame(dat_njstation) %>%
  filter(week == 17) %>%
  dplyr::select(lag_1day, lag_7day, lag14day, delay_minutes) %>%
  gather(Variable, Value, -delay_minutes) %>%
  mutate(Variable = fct_relevel(Variable, "lag_1day","lag_7day","lag_14day",
...
plotData.lag %>% 
  ...
  labs(title = "Bike Share Trip Count as a function of time lags",
       subtitle = "One week in December, 2018") +
  plotTheme
```
