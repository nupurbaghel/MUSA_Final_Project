...
dat_njstation <- dat_njstation %>% rename(dist = distance.X)
dat_njstation$dist = dat_njstation$dist/1609.344
```
...
dat_njstation <- cbind(dat_njstation, diff_delay_var$diff_delay)
```
...
```{r}
reg2 <- 
  lm(delay_minutes ~ from + to + total_stops + hour + dotw + train_id + Wind_Speed + Temperature + Precipitation + dist + lag_1day + lag_1week + lag_2week + congestion + critical, na.action = na.exclude, data = dat.Train)
...
require(caTools)
rf <- randomForest(
  delay_minutes ~ from + to + total_stops + hour + dotw + train_id + Wind_Speed + Temperature + Precipitation + dist + lag_1day + lag_1week + lag_2week + congestion + critical, na.action = na.exclude, data=dat.Train)
```
```{r}
summary(rf)
```
...
```
```{r}
unique(dat_njstation %>% filter(train_id %in% c('0209','0513','4306','6305')) %>% select(hour))
```
